...
public class DynamoDBClient extends DB {
   ...
   *
   * By default, the primary key type is "HASH". Optionally, the user can choose
   * to use hash_and_range key type. See documentation in the
  ...
  private static final Status CLIENT_ERROR =
      new Status("CLIENT_ERROR", "An error occurred on the client.");
  ...
  public void init() throws DBException {
    ...
    String debug = config.getProperty("dynamodb.debug", null);

    if (null != debug && "true".equalsIgnoreCase(debug)) {
      logger.setLevel(Level.DEBUG);
    }

    String configuredEndpoint = config.getProperty("dynamodb.endpoint", null);
    String credentialsFile =
        config.getProperty("dynamodb.awsCredentialsFile", null);
    String primaryKey = config.getProperty("dynamodb.primaryKey", null);
    String primaryKeyTypeString =
        config.getProperty("dynamodb.primaryKeyType", null);
    String consistentReads =
        config.getProperty("dynamodb.consistentReads", null);
    String connectMax = config.getProperty("dynamodb.connectMax", null);
    ...
    if (null != primaryKeyTypeString) {
      try {
        this.primaryKeyType =
            PrimaryKeyType.valueOf(primaryKeyTypeString.trim().toUpperCase());
    ...
    if (this.primaryKeyType == PrimaryKeyType.HASH_AND_RANGE) {
      ...
      String configuredHashKeyName =
          config.getProperty("dynamodb.hashKeyName", null);
      ...
      this.hashKeyValue =
          config.getProperty("dynamodb.hashKeyValue", DEFAULT_HASH_KEY_VALUE);
    }

    try {
      AWSCredentials credentials =
          new PropertiesCredentials(new File(credentialsFile));
      ...
      String errMsg =
          "DynamoDBClient.init(): Could not initialize DynamoDB client: "
              + e1.getMessage();
      logger.error(errMsg);
    }
  }
  ...
  public Status scan(String table, String startkey, int recordcount,
    ...
    logger.debug("scan " + recordcount + " records from key: " + startkey
        + " on table: " + table);
    /*
  ...
  public Status update(String table, String key,
    ...
    for (Entry<String, ByteIterator> val : values.entrySet()) {
      ...
      attributes.put(val.getKey(),
          new AttributeValueUpdate().withValue(v).withAction("PUT"));
    }

    UpdateItemRequest req =
        new UpdateItemRequest(table, createPrimaryKey(key), attributes);
  ...
  public Status insert(String table, String key,
      HashMap<String, ByteIterator> values) {
    logger.debug(
        "insertkey: " + primaryKeyName + "-" + key + " from table: " + table);
  ...
  private static Map<String, AttributeValue> createAttributes(
    ...
    Map<String, AttributeValue> attributes =
        new HashMap<String, AttributeValue>(values.size() + 1); // leave space
                                                                // for the
                                                                // PrimaryKey
    for (Entry<String, ByteIterator> val : values.entrySet()) {
      attributes.put(val.getKey(),
          new AttributeValue(val.getValue().toString()));
    }
    return attributes;
  }
  ...
  private HashMap<String, ByteIterator> extractResult(
    ...
    HashMap<String, ByteIterator> rItems =
        new HashMap<String, ByteIterator>(item.size());
    for (Entry<String, AttributeValue> attr : item.entrySet()) {
      logger.debug(String.format("Result- key: %s, value: %s", attr.getKey(),
          attr.getValue()));
  ...
  private Key createPrimaryKey(String key) {
    ...
    if (primaryKeyType == PrimaryKeyType.HASH) {
      ...
      k = new Key().withHashKeyElement(new AttributeValue().withS(hashKeyValue))
          .withRangeKeyElement(new AttributeValue().withS(key));
    } else {
      throw new RuntimeException(
          "Assertion Error: impossible primary key" + " type");
    }
    return k;
  }
}
