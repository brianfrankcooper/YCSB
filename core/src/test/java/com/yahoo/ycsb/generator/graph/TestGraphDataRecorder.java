...
public class TestGraphDataRecorder {
  ...
  public void creatingFilesTestInRunPhaseWithLoadFiles() throws IOException {
    GraphDataRecorder loadGraphDataRecorder = getGraphDataRecorderInLoadPhase();

    GraphDataRecorder runGraphDataRecorder = getGraphDataRecorderInRunPhase();

    Set<File> list = new HashSet<>(Arrays.asList(Objects.requireNonNull(directory.listFiles())));

    assertEquals(2, list.size());
    assertTrue(list.contains(runGraphDataRecorder.getGraphFile()));
    assertTrue(list.contains(loadGraphDataRecorder.getGraphFile()));
    assertNotSame(runGraphDataRecorder.getGraphFile(), loadGraphDataRecorder.getGraphFile());
  }
  ...
  @Test
  public void checkIfGraphComponentsCanBeRetrievedByGetInRunPhaseWithLoadFiles() throws IOException, NoSuchFieldException, IllegalAccessException {
    int numberOfNodes = 5;
    GraphDataRecorder loadGraphDataRecorder = getGraphDataRecorderInLoadPhase();

    for (int i = 0; i < numberOfNodes; i++) {
      loadGraphDataRecorder.nextValue();
    }

    TestGraphDataGenerator.resetStaticFieldsOfNodeAndEdge();
    ...
    assertEquals(numberOfNodes, graphList.get(0).getNodes().get(0).getId());
  }
  ...
  public void testLastValueInLoadPhase() throws IOException {
    ...
    Graph graph = graphDataRecorder.nextValue();
  ...
  private void checkCreatedGraphsWithGetter(GraphDataRecorder graphDataRecorder, List<Graph> graphList) {
    for (Graph graph : graphList) {
      for (Node node : graph.getNodes()) {
        assertEquals(node.getId(), graphDataRecorder.getNode(node.getId()).getId());
        assertEquals(node.getLabel(), graphDataRecorder.getNode(node.getId()).getLabel());
      }
      for (Edge edge : graph.getEdges()) {
        assertEquals(edge.getHashMap().toString(), graphDataRecorder.getEdge(edge.getId()).getHashMap().toString());
      }
    }
  }
}