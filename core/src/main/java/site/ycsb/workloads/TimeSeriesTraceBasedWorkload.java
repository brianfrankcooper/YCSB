 ...
 * </ul>
 */
public class TimeSeriesTraceBasedWorkload extends Workload {  
  ...
  /** Name and default value for the trace file path property. */
  public static final String TRACE_FILE_PATH_PROPERTY = "tracefilepath";
  public static final String TRACE_FILE_PATH_PROPERTY_DEFAULT = "workload_trace.txt";
  ...
  /** The path for the trace file */
  protected String traceFilePath;

  /** The BufferedReader for the trace file */
  protected BufferedReader traceFileReader;
  ...
  public void init(final Properties p) throws WorkloadException {
    ...
    // Open BufferedReader for file containing workload trace
    traceFilePath =
        p.getProperty(TRACE_FILE_PATH_PROPERTY, 
            TRACE_FILE_PATH_PROPERTY_DEFAULT);
    try {
      traceFileReader = new BufferedReader(new FileReader(traceFilePath));
    } catch (IOException e) {
      throw new WorkloadException("Error while opening trace file at path: " + traceFilePath, e);
    }
  }
  ...
  public boolean doTransaction(DB db, Object threadstate) {
    ...
    String nextTransaction = new String();
    try {
      nextTransaction = traceFileReader.readLine();
    } catch(IOException e) {
      System.err.println("Error while reading line from trace file at path: " + traceFilePath);
      e.printStackTrace();
      nextTransaction = null;
    }
    if (nextTransaction == null) {
      requestStop(); // we have reached the end of the file so workload should stop
    } else {
      // Example Transaction String "INSERT timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} value=8.000000";
      // Example Transaction String "READ timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB}";
      // Example Transaction String "SCAN startTimestamp=1000000000 endTimestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} groupbyfunction=SUM groupbytags={AAAA,BBBB}";
      // Example Transaction String "SCAN startTimestamp=1000000000 endTimestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} downsamplingfunction=SUM downsamplinginterval=3600";
      List<String> transactionParts = Arrays.asList(nextTransaction.split(" "));
      String transactionType = transactionParts.get(0);
      List<String> transactionParams = transactionParts.subList(1, transactionParts.size());
      System.out.println("[TimeSeriesTraceBasedWorkload][" + transactionType + "] " + transactionParams);
      switch (transactionType) {
      case "READ":
        doTransactionRead(db, threadstate, transactionParams);
        break;
      case "UPDATE":
        doTransactionUpdate(db, threadstate, transactionParams);
        break;
      case "INSERT": 
        doTransactionInsert(db, threadstate, transactionParams);
        break;
      case "SCAN":
        doTransactionScan(db, threadstate, transactionParams);
        break;
      case "DELETE":
        doTransactionDelete(db, threadstate, transactionParams);
        break;
      default:
        return false;
      }
  ...
  protected void doTransactionRead(final DB db, Object threadstate, List<String> transactionParams) {
    // Example Transaction String "READ timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB}";
    //System.out.println("doTransactionRead");
    final ThreadState state = (ThreadState) threadstate;
    // Additional query params for a db.read() call are stored in a Set<String>
    // with each string representing a Key/Value pair eg. YCSBTC=123456789
    final Set<String> fields = new HashSet<String>();
    final String metric = parseTransactionParams(transactionParams, fields);
    final Map<String, ByteIterator> result = new HashMap<String, ByteIterator>();
    final Status status = db.read(table, metric, fields, result);
  }
  
  protected void doTransactionUpdate(final DB db, Object threadstate, List<String> transactionParams) {
    if (threadstate == null) {
      throw new IllegalStateException("Missing thread state.");
    }
    final Map<String, ByteIterator> tags = new TreeMap<String, ByteIterator>();
  ...
  protected void doTransactionInsert(final DB db, Object threadstate, List<String> transactionParams) {
    //System.out.println("doTransactionInsert");
    // Example Transaction String "INSERT timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} value=8.000000";
    final Map<String, ByteIterator> tags = new TreeMap<String, ByteIterator>();
    final String metric = parseTransactionParamsInsert(transactionParams, tags);
    //System.out.println("just before db.insert() --> metric: " + metric + ", tags: " + tags);
    db.insert(table, metric, tags);
  }
  
  protected void doTransactionScan(final DB db, Object threadstate, List<String> transactionParams) {
    final ThreadState state = (ThreadState) threadstate;
    // Additional query params for a db.scan() call are stored in a Set<String>
    // with each string representing a Key/Value pair eg. YCSBTC=123456789
    final Set<String> fields = new HashSet<String>();
    final String metric = parseTransactionParams(transactionParams, fields);
    final Vector<HashMap<String, ByteIterator>> results = new Vector<HashMap<String, ByteIterator>>();
    // Note the 3rd parameter is not used - normally it represents the length of the "scan" in a
    // traditional database sense of scanning through keys/rows, starting from a particular key.
    // In the time series context we choose a scan to mean a scan of values from a start timestamp
    // to an end timestamp. The start and end timestamp are encoded with special tags in the fields Set.
    db.scan(table, metric, 0, fields, results);
  }
  
  protected void doTransactionDelete(final DB db, Object threadstate, List<String> transactionParams) {
    final ThreadState state = (ThreadState) threadstate;
  ...
  /**
   * Parses the parts of an insert transaction read from a line out of the workload trace file
   * @param transactionParams A list of strings containing the transaction parameters read from the trace file
   * @param tags An initialized map to populate with tag keys and values as well
   * as the timestamp and actual value.
   * @return The metric key for the transaction.
   */
  protected String parseTransactionParamsInsert(List<String> transactionParams, final Map<String, ByteIterator> tags) {
    // The transactionParams is the part that comes after the transaction type (eg. INSERT/READ/SCAN),
    // and is just that string split on the space character.
    // Example Transaction String "INSERT timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} value=8.000000";
    //System.out.println("Parsing parameters...");
    // Go through all the transacation parameters
    String metric = new String();
    for (String paramPair : transactionParams) {
      String[] paramPairParts = paramPair.split("=");
      String paramKey = paramPairParts[0];
      String paramValue = paramPairParts[1];
      //System.out.println("paramKey: " + paramKey);
      //System.out.println("paramValue: " + paramValue);
      if (paramKey.equals("timestamp")) {
        tags.put(timestampKey, new NumericByteIterator(new Long(paramValue)));
      } else if (paramKey.equals("tags")) {
        Arrays.stream(paramValue.replaceAll("\\{|\\}", "").split(",")).forEach(tagPair -> {
          //System.out.println("tagPair = " + tagPair);
          String[] tagPairParts = tagPair.split(":");
          //System.out.println("tagPairParts = " + tagPairParts);
          tags.put(tagPairParts[0], new StringByteIterator(tagPairParts[1]));
        });
      } else if (paramKey.equals("value")) {
        tags.put(valueKey, new NumericByteIterator(new Double(paramValue)));
      } else if (paramKey.equals("metric")) {
        metric = paramValue;
  ...
  /**
   * Parses the parts of a read/scan transaction read from a line out of the workload trace file
   * @param transactionParams A list of strings containing the transaction parameters read from the trace file
   * @param fields An initialized set to populate with tag keys and values as well
   * as the timestamp, and a groupbyfunction or downsampling function if relevant .
   * @return The metric key for the transaction.
   */
  protected String parseTransactionParams(List<String> transactionParams, final Set<String> fields) {
    // The transactionParams is the part that comes after the transaction type (eg. READ/SCAN),
    // and is just that string split on the space character.
    // Example Transaction String "READ timestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB}";
    // Example Transaction String "SCAN starttimestamp=1000000000 endtimestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} groupbyfunction=SUM groupbytags={AAAA,BBBB}";
    // Example Transaction String "SCAN starttimestamp=1000000000 endtimestamp=1000000000 metric=AAAAA tags={AAAA:AAAA,BBBB:BBBB} downsamplingfunction=SUM downsamplinginterval=3600";
    ...
    // Go through all the transacation parameters
    String metric = new String();
    String downsampleFunction = new String();
    String downsampleInterval= new String();
    String startTimestamp = new String();
    String endTimestamp = new String();
    for (String paramPair : transactionParams) {
      String[] paramPairParts = paramPair.split("=");
      String paramKey = paramPairParts[0];
      String paramValue = paramPairParts[1];
      ...
      //System.out.println("paramValue: " + paramValue);
      if (paramKey.equals("timestamp")) {
        fields.add(timestampKey + tagPairDelimiter + paramValue); 
      } else if (paramKey.equals("starttimestamp")) {
        startTimestamp = paramValue;
      } else if (paramKey.equals("endtimestamp")) {
        ...
        Arrays.stream(paramValue.replaceAll("\\{|\\}", "").split(",")).forEach(tagPair -> {
          ...
          String[] tagPairParts = tagPair.split(":");
          //System.out.println("tagPairParts = " + tagPairParts);
          fields.add(tagPairParts[0] + tagPairDelimiter + tagPairParts[1]);
        });
      } else if (paramKey.equals("groupbyfunction")) {
        fields.add(groupByKey + tagPairDelimiter + paramValue);
      } else if (paramKey.equals("groupbytags")) {
        Arrays.stream(paramValue.replaceAll("\\{|\\}", "").split(",")).forEach(tag -> {
          // GroupBy Tags are just represented in the list of fields as tags with no value
          fields.add(tag);
        });
      // IMPORTANT: In the transaction in the trace file, the downsamplingfunction must come BEFORE the downsamplinginterval!
      } else if (paramKey.equals("downsamplingfunction")) {
        downsampleFunction = paramValue;
      } else if (paramKey.equals("downsamplinginterval")) {
        downsampleInterval = paramValue;
      } else if (paramKey.equals("metric")) {
        metric = paramValue;
        //System.out.println("metric: " + metric);
      }
    }
    // Add field for combined downsampling code if both values were parsed
    // The code is just a concatenation of the function and the time interval,
    // eg. for SUM and 3600 the code = "SUM3600"
    if (!downsampleFunction.isEmpty() && !downsampleInterval.isEmpty()) {
      fields.add(downsampleKey + tagPairDelimiter + downsampleFunction + downsampleInterval);
    }
    // Add the timestamp range (if both values were parsed)
    // NOTE: If for some reason a normal timestamp= entry was also present in the transacation then the timestamp
    // range will not be added to the set because it uses the same key as a normal timestamp, just with a
