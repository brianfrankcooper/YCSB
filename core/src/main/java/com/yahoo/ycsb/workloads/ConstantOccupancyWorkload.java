 ...
 * <a href= 'https://database.cs.wisc.edu/cidr/cidr2007/papers/cidr07p34.pdf'>
 * Fragmentation in Large Object Repositories</a>, CIDR 2006. [<a
 * href='https://database.cs.wisc.edu/cidr/cidr2007/slides/p34-sears.ppt'>
 * Presentation</a>] </p>
 *
 *
 * @author sears
...
public class ConstantOccupancyWorkload extends CoreWorkload {
  ...
  private IntegerGenerator objectsizes;
  ...
  public void init(final Properties p) throws WorkloadException {
    disksize = Long.parseLong(
        p.getProperty(DISK_SIZE_PROPERTY, DISK_SIZE_PROPERTY_DEFAULT + ""));
    storageages = Long.parseLong(
        p.getProperty(STORAGE_AGE_PROPERTY, STORAGE_AGE_PROPERTY_DEFAULT + ""));
    occupancy = Double.parseDouble(
        p.getProperty(OCCUPANCY_PROPERTY, OCCUPANCY_PROPERTY_DEFAULT + ""));
    ...
    if ((p.getProperty(Client.RECORD_COUNT_PROPERTY) != null)
      ...
      System.err.println(
          "Warning: record, insert or operation count was set prior to "
              + "initting ConstantOccupancyWorkload.  Overriding old values.");
    }
    final IntegerGenerator g = CoreWorkload.getFieldLengthGenerator(p);
    final double fieldsize = g.mean();
    final int fieldcount = Integer.parseInt(
    ...
    if (objectCount == 0) {
      throw new IllegalStateException(
          "Object count was zero.  Perhaps disksize is too low?");
    }
    p.setProperty(Client.RECORD_COUNT_PROPERTY, objectCount + "");
    p.setProperty(Client.OPERATION_COUNT_PROPERTY,
        (storageages * objectCount) + "");
    p.setProperty(Client.INSERT_COUNT_PROPERTY, objectCount + "");
