...
public abstract class Workload {
   ...
   * have no side effects other than DB operations and mutations on threadstate.
   * Mutations to threadstate do not need to be synchronized, since each thread
   * has its own threadstate instance.
   */
   ...
   *         terminate the thread. Return true otherwise. Return true for
   ...
   * state that is specific to one thread. To be clear, this means the returned
   * object should be created anew on each call to initThread(); do not return
   * the same object multiple times. The returned object will be passed to
   * invocations of doInsert() and doTransaction() for this thread. There should
   * be no side effects from this call; all state should be encapsulated in the
   * returned object. If you have no state to retain for this thread, return
   * null. (But if you have no state to retain for this thread, probably you
   ...
   *         when you are done.
   */
  public Object initThread(final Properties p, final int mythreadid,
      final int threadcount) throws WorkloadException {
    return null;
  }
