...
public class GoodBadUglyDB extends DB {
  ...
  public static final String SIMULATE_DELAY_DEFAULT =
  ...
  public void init() {
    int i = 0;
    for (final String delay : getProperties()
        .getProperty(SIMULATE_DELAY, SIMULATE_DELAY_DEFAULT).split(",")) {
  ...
  void delay() {
    ...
    final Lock lock = mod == 3 ? DB_ACCESS.writeLock() : DB_ACCESS.readLock();
    if (mod == 3) {
    ...
    try {
      ...
      final int delayRangeNs =
          (int) (MICROSECONDS.toNanos(delays[mod + 1]) - baseDelayNs);
