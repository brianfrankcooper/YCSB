...
public abstract class ByteIterator implements Iterator<Byte> {
   ...
   */
  public int nextBuf(final byte[] buf, final int bufOffset) {
    int sz = bufOffset;
   ...
   */
  public byte[] toArray() {
    final long left = bytesLeft();
    if (left != (int) left) {
      throw new ArrayIndexOutOfBoundsException(
          "Too much data to fit in one array!");
    }
    final byte[] ret = new byte[(int) left];
    int off = 0;
  ...
  public String toString() {
    ...
    final CharBuffer cb = cset.decode(ByteBuffer.wrap(this.toArray()));
    return cb.toString();
  }
