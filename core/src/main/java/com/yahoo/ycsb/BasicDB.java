...
public class BasicDB extends DB {
  ...
  public void init() {
    verbose = Boolean
        .parseBoolean(getProperties().getProperty(VERBOSE, VERBOSE_DEFAULT));
    todelay = Integer.parseInt(
        getProperties().getProperty(SIMULATE_DELAY, SIMULATE_DELAY_DEFAULT));
    randomizedelay = Boolean.parseBoolean(
        getProperties().getProperty(RANDOMIZE_DELAY, RANDOMIZE_DELAY_DEFAULT));
    if (verbose) {
   ...
   */
  @Override
  public Status scan(final String table, final String startkey,
    ...
    if (verbose) {
      System.out
          .print("SCAN " + table + " " + startkey + " " + recordcount + " [ ");
      if (fields != null) {
        for (final String f : fields) {
          System.out.print(f + " ");
        }
  ...
  void delay() {
    if (todelay > 0) {
      ...
      if (randomizedelay) {
        delayNs =
            TimeUnit.MILLISECONDS.toNanos(Utils.random().nextInt(todelay));
        if (delayNs == 0) {
          return;
        }
      } else {
      ...
      do {
        ...
        now = System.nanoTime();
      } while ((now < deadline) && !Thread.interrupted());
    }
  }
}
