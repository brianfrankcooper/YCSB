		...
		for (int i = 0; i < OperationRetries; i++) {
			...
			try {
				...
				for (ColumnOrSuperColumn oneresult : results) {
					...
					String name = new String(column.name.array(),
							column.name.position() + column.name.arrayOffset(),
							column.name.remaining());
					String value = new String(column.value.array(),
	...
	public int scan(String table, String startkey, int recordcount,
		...
		for (int i = 0; i < OperationRetries; i++) {
			...
			try {
				...
				List<KeySlice> results = client.get_range_slices(parent,
						predicate, kr, ConsistencyLevel.ONE);
	...
	public int insert(String table, String key, HashMap<String, String> values) {
		...
		for (int i = 0; i < OperationRetries; i++) {
			...
			try {
				...
				batch_mutation.put(ByteBuffer.wrap(key.getBytes("UTF-8")),
						cfMutationMap);

				for (String field : values.keySet()) {
					String val = values.get(field);
					Column col = new Column(ByteBuffer.wrap(field
							.getBytes("UTF-8")), ByteBuffer.wrap(val
							.getBytes("UTF-8")), timestamp);
