...
public class CassandraClient5 extends DB {
	...
	public void init() throws DBException {
		...
		if (hosts == null) {
			throw new DBException(
					"Required property \"hosts\" missing for CassandraClient5");
		}

		ConnectionRetries = Integer.parseInt(getProperties().getProperty(
				CONNECTION_RETRY_PROPERTY, CONNECTION_RETRY_PROPERTY_DEFAULT));
		OperationRetries = Integer.parseInt(getProperties().getProperty(
				OPERATION_RETRY_PROPERTY, OPERATION_RETRY_PROPERTY_DEFAULT));

		_debug = Boolean.parseBoolean(getProperties().getProperty("debug",
				"false"));
	...
	public int read(String table, String key, Set<String> fields,
		...
		for (int i = 0; i < OperationRetries; i++) {
			...
			try {
				...
				if (fields == null) {
					predicate = new SlicePredicate(null, new SliceRange(
							new byte[0], new byte[0], false, 1000000));
				} else {
				...
				List<ColumnOrSuperColumn> results = client.get_slice(table,
				...
				for (ColumnOrSuperColumn oneresult : results) {
					...
					if (_debug) {
						System.out.print("(" + new String(column.name) + "="
								+ new String(column.value) + ")");
					}
				}
	...
	public int scan(String table, String startkey, int recordcount,
		...
		for (int i = 0; i < OperationRetries; i++) {
			...
			try {
				...
				List<KeySlice> results = client.get_range_slice(table, parent,
						predicate, startkey, "", recordcount,
				...
				for (KeySlice oneresult : results) {
					...
					for (ColumnOrSuperColumn onecol : oneresult.columns) {
						...
						tuple.put(new String(column.name), new String(
								column.value));
	...
	public int delete(String table, String key) {
		...
		for (int i = 0; i < OperationRetries; i++) {
			try {
				client.remove(table, key, new ColumnPath("data", null, null),
	 ...
	 * 
	 * 
	 * // read entire row SlicePredicate predicate = new SlicePredicate(null,
