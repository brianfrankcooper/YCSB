...
public class ElasticsearchRestClient extends DB {
  ...
  private static Logger log = LogManager.getLogger(ElasticsearchRestClient.class);
  ...
  public void init() throws DBException {
    ...
    try {
      ...
      exists = existsResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK;
    } catch (IOException e) {
  ...
  private static Response performRequest(
    ...
    if (entity != null) {
      ...
      builder.addHeader("content-type", ContentType.APPLICATION_JSON.toString());
  ...
  public Status insert(final String table, final String key, final Map<String, ByteIterator> values) {
    try {
      ...
      final Response response = performRequest(
          restClient,
          "POST",
          "/" + indexKey + "/_doc/",
  ...
  public Status delete(final String table, final String key) {
    try {
      final Response searchResponse = search(key);
      ...
      @SuppressWarnings("unchecked") final Map<String, Map<String, Object>> hits
          = (Map<String, Map<String, Object>>)map.get("hits");
      final int total = (int)hits.get("total").get("value");
      if (total == 0) {
        return Status.NOT_FOUND;
      }
  ...
  private Response search(final String key) throws IOException {
    try (XContentBuilder builder = jsonBuilder()) {
      builder.startObject();
      builder.startObject("query");
  ...
  private Response search(final XContentBuilder builder) throws IOException {
    refreshIfNeeded();
    final Map<String, String> params = emptyMap();
    final StringEntity entity = new StringEntity(Strings.toString(builder));
