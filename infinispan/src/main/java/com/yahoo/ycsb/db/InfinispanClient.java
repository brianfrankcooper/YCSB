...
public class InfinispanClient extends DB {
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    try {
      ...
      if (clustered) {
        row = AtomicMapLookup.getAtomicMap(infinispanManager.getCache(table),
            key, false);
      } else {
        Cache<String, Map<String, String>> cache =
            infinispanManager.getCache(table);
        row = cache.get(key);
      }
      if (row != null) {
        result.clear();
  ...
  public Status update(String table, String key,
    ...
    try {
      if (clustered) {
        AtomicMap<String, String> row = AtomicMapLookup
            .getAtomicMap(infinispanManager.getCache(table), key);
